...
km_vec=normrnd(21562,4000,1000,1); %fuel price sensitivity
...
for z=1:length(km_vec) %life sensitivity case
    
%for real NPV calcs
Bl_yearly=(BE-dE)*km_vec(z)*FP/100+0+0;
...
baseline_TCO=pvvar([PP ones(1,life)*Bl_yearly],dr(end)); %for lifetime sensitivity case
    ...
    %             reduced_TCO(j)=PP+(BE-dE_choice)*FP*km*life/100+wt_raw(j); 
                yearly=(BE-dE_choice)*km_vec(z)*FP/100;
...
if km_sens %plot the lifetime sensitivity results
...
%     set(hbar,'FaceColor','k');
    ...
    figure
    lt=cell2mat(ltvec);
    plot(km_vec,lt(1:2:end),'.k','MarkerSize',11);
    hold
    plot(km_vec,lt(2:2:end),'.r','MarkerSize',11);
    
    xlabel('Yearly driving distance (km)');
    ylabel('Optimal total cost of ownership')
    legend({'1000' '10,000'});
    
    figure
    rv=cell2mat(realvec);
    plot(km_vec,rv(1:2:end),'.k','MarkerSize',11);
    hold
    plot(km_vec,rv(2:2:end),'.r','MarkerSize',11);
    xlabel('Yearly driving distance (km)');
