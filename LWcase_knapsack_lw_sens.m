...
%     lw_cost_vec=ones(20,1).*sigvec(gg); %for the case where the
%     distribution is calculated with a mean of the technology cost
...
%     high=1+sigvec(gg);
%     
%     lw_cost_vec = low + (high-low).*rand(20,1000);

    %steps for the life vector
    % steps=length(life_vec);
    steps=length(lw_cost_vec);
...
%     lw(indze,2)=1;
    ...
    for z=1:length(lw_cost_vec) %for montecarlo lw_cost_vec case
%     for z=1 %for deterministic lw_cost_vec case
...
%                     wt_raw(j)=normrnd(lw(j,2),sigvec(gg),1,1); %store the un-rounded up-front cost
    ...
    %             reduced_TCO(j+1)=pvvar([PP+wt_raw(j+1) ones(1,life_vec(z))*yearly],dr(end)); %for the life sensitivity case
                ...
                %find values which are below 0, indicating where LW
                %measures wouldn't be selected
                ind_val_neg=find(value<=0);
                num_below(i)=length(ind_val_neg);

                edit_weights{i}=weight; 
                edit_values{i}=value;
                
                tot_weight(i)=sum(edit_weights{i});
...
%             keyboard
            ...
            sel_tot_wt=tot_weight(maxind);
            max_tot_weight(s)=sel_tot_wt;
    ...
    %         keyboard
        ...
        max_sum_tot_weight(z)=max_tot_weight;
...
%         ylabel('Optimal total cost of ownership')
        ...
        bar_data=[length(num_none) length(num_dst) length(num_ss) length(num_full) length(num_plug) length(num_ev)];
        hbar=bar(bar_data);
    ...
    lower(gg)=mean(num_low_than0);
    maxTotWt{gg}=max_sum_tot_weight;
end
...
figure
plot(sigvec,lower,'ko')
xlabel('Cost uncertainty (\sigma) ')
ylabel('Mean number of lightweighting technologies with negative value')
...
%plot max selected value/weight ratios for all of the 1000 trials - trying
%to figure out what was leading to cost uncertaintly
% close all
